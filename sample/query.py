...
def search_and_order(stock):
    item_name = input("What is the name of the item? ").lower()
    total_available = sum(1 for item in stock if item_name.lower() in f"{item['state']} {item['category']}".lower())
    
    if total_available == 0:
        print("Amount available: 0\nLocation: Not in stock")
    else:
        print(f"Total amount available of this item is: {total_available}")
    ...
    if order == 'y':
        ...
        if desired_amount <= total_available:
            ...
            if order_max == 'y':
                available_per_warehouse = {}
                for item in stock:
                    if item_name.lower() in f"{item['state']} {item['category']}".lower():
                        available_per_warehouse[item['warehouse']] = available_per_warehouse.get(item['warehouse'], 0) + 1
                max_warehouse = max(available_per_warehouse, key=available_per_warehouse.get)
                print(f"{available_per_warehouse[max_warehouse]} {item_name} have been ordered from Warehouse {max_warehouse}.")
            else:
                print(f"{desired_amount} {item_name} have not been ordered.")
